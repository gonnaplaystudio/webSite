<?php
    ...
    $sql_games="select * from inf_games_es";
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top pegajosa nav-bar-coll">
  ...
  <div class="container-md">
    ...
    <form  method="POST" action="admin_mod_game.php">
      <div class="row justify-content-md-center">  
      <div class="col-lg-6">
          <fieldset>
            <div class="form-border-back">
              <legend class="tittle-form-back">SELECCIONA EL JUEGO A MODIFICAR</legend>
                <div class="form-group contenedor-inputs text-center">

                  <div class="form-group i-100">
                    <select class="form-select" id="select_game" name="select_game">
                      <?php 
                        while($rows_games=mysqli_fetch_assoc($rs_games))
                        {
                      ?>
                      <option><?php echo $rows_games["alias"] ?></option>
                      <?php }
                        mysqli_close($con);
                      ?>
                    </select>
